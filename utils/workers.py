        ...
        while grabbed_count < self.target_count and not self._is_stopped:
            ...
            try:
                ...
                if result.get("code") == 1000:
                    ...
                    # 检查是否是特定的错误类型，决定是否需要特殊处理
                    if "JAS-04" in msg or "校验不通过" in msg:
                        self.status_update.emit(self.sid, f"⚠️ 《{course_name}》 遇到会话校验问题: {msg}，API已自动重试")
                    elif "容量满" in msg or "人数已满" in msg:
                        self.status_update.emit(self.sid, f"📵 《{course_name}》 课程已满: {msg}，尝试下一志愿...")
                    elif "时间冲突" in msg:
                        self.status_update.emit(self.sid, f"⏰ 《{course_name}》 时间冲突: {msg}，尝试下一志愿...")
                    else:
            ...
            if not self._is_stopped:
                # 动态调整延时：如果遇到校验问题，延长等待时间
                if "JAS-04" in msg or "校验不通过" in msg:
                    time.sleep(5)  # 校验问题时等待更长时间
                else:
                    time.sleep(1.5)  # 正常情况下的延时
