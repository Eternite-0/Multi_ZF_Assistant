...
class Client:
    ...
    def __init__(self, cookies={}, **kwargs):
        ...
        # 选课页面里的 hidden 参数会随会话刷新而变化；课程缓存中的
        # do_id/class_id 是稳定标识，JAS-04 时只刷新页面上下文，不重复查询课程。
        self._cached_hidden = {}
        self._selection_context_at = 0.0
        self._selection_context_lock = threading.RLock()
        self._selection_refresh_lock = threading.Lock()
        self._selection_entry_url = ""
        self._last_selection_refresh_ok = False
    ...
    def _get_course_entry_page_doc(self):
        ...
        url_entry = urljoin(
            self.base_url,
            f"xsxk/zzxkyzb_cxZzxkYzbIndex.html?gnmkdm=N253512&layout=default&_t={int(time.time() * 1000)}",
        )
    ...
    def _load_selection_context(self):
        """重新加载选课入口和 Display 页面，返回本次会话的 hidden 参数。

        JAS-04 不仅可能表示登录 Cookie 过期，也可能表示选课页面上下文
        已失效。每次刷新都要重新访问入口页和 Display 页，不能只更新本地
        字典或等待后重发旧请求。
        """
        context_lock = getattr(self, "_selection_context_lock", None)
        if context_lock is None:
            context_lock = threading.RLock()
            self._selection_context_lock = context_lock
        with context_lock:
            ...
            display_url = urljoin(
                self.base_url,
                f"xsxk/zzxkyzb_cxZzxkYzbDisplay.html?gnmkdm=N253512&_t={int(time.time() * 1000)}",
            )
            self._safe_request(
                "GET",
                display_url,
            ...
            if not hidden:
                raise exceptions.RequestException("选课入口页面未返回动态参数。")

            self._cached_hidden = hidden
            self._selection_context_at = time.monotonic()
            self._selection_entry_url = entry_response.url
            headers = getattr(self, "headers", None)
            if headers is not None:
                headers["Referer"] = entry_response.url
            return hidden, entry_response

    def _dynamic_selection_values(self, course_data=None):
        """读取当前页面参数；课程 cache 只作为旧会话兜底。"""
        hidden = getattr(self, "_cached_hidden", {}) or {}
        ...
        return {
            "kklxdm": value("firstKklxdm", "kklxdm") or course_data.get("kklxdm"),
            "xkkz_id": value("firstXkkzId", "xkkz_id") or course_data.get("xkkz_id"),
        }

    def _refresh_course_operation_id(self, course_data: dict):
        """只刷新当前教学班的操作令牌，不重新扫描整个课程列表。

        ``course_id``/``class_id`` 是稳定标识，但部分正方部署会在每次
        ``cxJxbWithKchZzxkYzb`` 响应中重新生成 ``do_jxb_id``。该字段实际
        是选课请求的会话绑定操作令牌，长期写入 cache 后会触发 JAS-04。
        正常抢课路径不调用本方法；仅在 JAS-04 恢复时按当前课程精准查询，
        以兼顾速度和令牌有效性。
        """
        ...
        if not hidden:
            try:
                hidden, _ = self._load_selection_context()
            except Exception:
                return None
        ...
        payload = {
            "rwlx": value("rwlx", default="1"),
            "xklc": value("xklc", default="1"),
            "xkly": value("xkly", default="0"),
            "bklx_id": value("bklx_id", default="0"),
            "sfkkjyxdxnxq": value("sfkkjyxdxnxq", default="0"),
            "kzkcgs": value("kzkcgs", default="0"),
            "xqh_id": value("xqh_id"),
            "jg_id": value("jg_id_1", "jg_id"),
            "njdm_id_1": value("njdm_id_1"),
            "zyh_id_1": value("zyh_id_1"),
            "zyh_id": value("zyh_id"),
            "zyfx_id": value("zyfx_id"),
            "njdm_id": value("njdm_id"),
            "bh_id": value("bh_id"),
            "bjgkczxbbjwcx": value("bjgkczxbbjwcx", default="0"),
            "xbm": value("xbm"),
            "xslbdm": value("xslbdm"),
            "mzm": value("mzm"),
            "xz": value("xz"),
            "ccdm": value("ccdm"),
            "xsbj": value("xsbj", default="0"),
            "sfkknj": value("sfkknj", default="0"),
            "gnjkxdnj": value("gnjkxdnj", default="0"),
            "sfkkzy": value("sfkkzy", default="0"),
            "kzybkxy": value("kzybkxy", default="0"),
            "sfznkx": value("sfznkx", default="0"),
            "zdkxms": value("zdkxms", default="0"),
            "sfkxq": value("sfkxq", default="0"),
            "njdm_id_xs": value("njdm_id_xs"),
            "zyh_id_xs": value("zyh_id_xs"),
            "sfkcfx": value("sfkcfx", default="0"),
            "kkbk": value("kkbk", default="0"),
            "kkbkdj": value("kkbkdj", default=""),
            "bklbkcj": value("bklbkcj", default="0"),
            "sfkgbcx": value("sfkgbcx", default="0"),
            "sfrxtgkcxd": value("sfrxtgkcxd", default="0"),
            "tykczgxdcs": value("tykczgxdcs", default="0"),
            "xkxnm": value("xkxnm"),
            "xkxqm": value("xkxqm"),
            "bbhzxjxb": value("bbhzxjxb", default="0"),
            "bhbcyxkjxb": value("bhbcyxkjxb", default=""),
            "rlkz": value("rlkz", default="0"),
            "xkzgbj": value("xkzgbj", default="0"),
            "zxgbxkkg": value("zxgbxkkg", default=""),
            "txbsfrl": value("txbsfrl", default="0"),
            "xkxskcgskg": value("xkxskcgskg", default="0"),
            "cdrlkz": value("cdrlkz", default="0"),
            "rlzlkz": value("rlzlkz", default="1"),
            "jxbzcxskg": value("jxbzcxskg", default="0"),
            "cxcykclxxskg": value("cxcykclxxskg", default="0"),
            "kklxdm": kklxdm,
            "xkkz_id": xkkz_id,
            "kch_id": course_id,
        ...
        try:
            response = self._safe_request(
                ...
                data=payload,
                headers={
                    "Referer": getattr(self, "_selection_entry_url", "")
                    or urljoin(self.base_url, "xsxk/zzxkyzb_cxZzxkYzbIndex.html?gnmkdm=N253512"),
        ...
        if not isinstance(rows, list):
            return None
        matched = next((row for row in rows if str(row.get("jxb_id")) == str(class_id)), None)
        fresh_do_id = matched.get("do_jxb_id") if matched else None
        if not fresh_do_id:
            return None

        # 原地更新，让工作线程的当前志愿和后续轮询都使用新令牌；稳定 ID
        # 与课程缓存中的其它字段保持不变。
        course_data["do_id"] = fresh_do_id
        course_data["kklxdm"] = kklxdm
        course_data["xkkz_id"] = xkkz_id
        return course_data
    ...
    def get_block_courses(self, block: int = 1):
        ...
        """
        try:
            # 1. 访问选课主页获取必要隐藏参数（Index + Display 页面）
            try:
                hidden, entry_response = self._load_selection_context()
            ...
            for course in course_rows:
                ...
                if isinstance(jxb_list, list):
                    for jxb in jxb_list:
                        ...
                        all_classes.append({
                            "course_id": kch_id,
                            # 页面展示的课程代码（例如 77103130）与
                            # course_id 内部主键不同，供手动志愿匹配使用。
                            "course_code": course.get("kch") or kch_id,
    ...
    def select_course(self, course_data: dict, _jas_attempt: int = 0):
        ...
        try:
            ...
            # 请求头
            ajax_headers = {
                'Referer': getattr(self, '_selection_entry_url', '') or urljoin(self.base_url, "xsxk/zzxkyzb_cxZzxkYzbIndex.html?gnmkdm=N253512"),
                'X-Requested-With': 'XMLHttpRequest',
            ...
            # kklxdm/xkkz_id 属于本次选课页面上下文，JAS-04 刷新后必须以
            # 最新 hidden 为准；course_data 中的值来自 cache，只用于没有
            # 页面上下文时兜底。课程/教学班 ID 则始终保留 cache 中的值。
            kklxdm = hidden_value("firstKklxdm", "kklxdm") or course_data.get("kklxdm")
            xkkz_id = hidden_value("firstXkkzId", "xkkz_id") or course_data.get("xkkz_id")
            if not kklxdm or not xkkz_id:
            ...
            if jas_error:
                # 一次接口调用只做一次页面刷新；外层持续轮询会负责后续
                # 重试，避免 API 与工作线程重复刷新同一个会话。
                if _jas_attempt < 1:
                    refreshed = self._refresh_session()
                    # 课程/教学班 ID 继续使用志愿 cache；刷新得到的 hidden
                    # 参数由下一次 select_course 直接读取。旧 cache 中的
                    # do_id 也可能是已过期的会话操作令牌；只对当前教学班
                    # 做一次精准刷新，不重新扫描整套课程列表。
                    if not refreshed:
                        return {
                            "code": 1010,
                            ...
                            "msg": f"选课会话刷新失败（JAS-04）: {msg}",
                        }

                    # 更新当前内存中的志愿对象：课程/教学班稳定 ID 不变，
                    # 但 do_id 是本次会话对应的动态操作令牌。这样下一轮
                    # 轮询也不会再次拿旧令牌触发 JAS-04。
                    refreshed_course = (
                        self._refresh_course_operation_id(course_data)
                        or course_data
                    )
                    # 保证工作线程持有的志愿对象也拿到新令牌；否则下一轮
                    # 可能继续使用旧 do_id。真实实现会原地更新，兼容返回
                    # 新字典的实现/测试桩时也统一回写到原对象。
                    if refreshed_course is not course_data and isinstance(refreshed_course, dict):
                        course_data.clear()
                        course_data.update(refreshed_course)
                        refreshed_course = course_data
                    time.sleep(0.5 + 0.5 * _jas_attempt)
                    return self.select_course(refreshed_course, _jas_attempt + 1)
                return {
                    "code": 1010,
                    "retryable": True,
                    "session_refreshed": _jas_attempt > 0,
    ...
    def _refresh_session(self):
        ...
        """
        # 同一账号只有一个刷新流程，避免多个并发抢课线程互相覆盖页面上下文。
        refresh_lock = getattr(self, "_selection_refresh_lock", None)
        if refresh_lock is None:
            refresh_lock = threading.Lock()
            self._selection_refresh_lock = refresh_lock
        with refresh_lock:
            try:
                main_url = urljoin(
                    self.base_url,
                    f"xtgl/index_initMenu.html?_t={int(time.time() * 1000)}",
                )
                self._safe_request('GET', main_url)
                self._load_selection_context()
                self._last_selection_refresh_ok = True
                return True
            except Exception as e:
                self._last_selection_refresh_ok = False
                print(f"刷新会话时出错: {e}")
